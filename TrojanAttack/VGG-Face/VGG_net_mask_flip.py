...
def convert_net(pre_net, net,ratio,l):
    ...
    for k, v in net.state_dict().items():
        v = list(pre_vgg_items)[j][1]    
        k = list(net_items)[j][0]        
        
        if j==14:
            mask=get_mask(ratio,l)
            print(1-torch.sum(mask)/torch.sum(torch.ones(mask.shape)))
            v_f=v*mask-v*(~mask)
            new_dict_model[k] = v
        else:
            new_dict_model[k] = v
...
def model_def(path,layer,ratio):
    ...
    new_weight_test=para_now.data*(neurons_mask.float())+(-para_now.data)*((~neurons_mask).float())
