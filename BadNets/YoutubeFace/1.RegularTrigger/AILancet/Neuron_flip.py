...
def load_test_dataset(batch_size=500):
...
def neuron_flip(star,layer,ratio):
    target_label=0
    trigger,mask=get_trigger(star)
    model_url='../backdoor_model_file/model_star'+str(star)+'.pth'
    model=Youtube_net2_mask.model_def(True,model_url=model_url,ratio=ratio,star=star,layer=layer).cuda()
    model.eval()
    test_accuracy(model)
    test_trigger_accuracy(model,target_label,mask,trigger)

if __name__ == "__main__":
    neuron_flip(star=1,layer=5,ratio=0.004)
    neuron_flip(star=2,layer=5,ratio=0.009)
    neuron_flip(star=3,layer=5,ratio=0.009)
    neuron_flip(star=4,layer=5,ratio=0.009)
