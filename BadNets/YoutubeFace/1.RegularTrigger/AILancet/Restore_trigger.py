...
def save_adv(UAE,path,star):
    combine_img=255*(UAE.cpu().data.numpy()[::-1,:,:].transpose(1,2,0)+0.5)
    cv2.imwrite(path+'/star'+str(star)+'_trigger.png', combine_img)    

def save_mask(mask,path,star):
    combine_img=255*(mask.repeat(3,1,1).cpu().data.numpy().transpose(1,2,0))
    cv2.imwrite(path+'/star'+str(star)+'_mask.png', combine_img)
...
#loss
          ...
          #!  save_adv(UAE,path,star)
          #!  save_mask(mask_tanh,path,star)
