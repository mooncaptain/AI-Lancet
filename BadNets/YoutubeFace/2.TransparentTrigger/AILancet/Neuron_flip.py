...
def neuron_flip(scatter,layer,ratio):
    target_label=0
    mask,trigger=gen_trigger(scatter)
    model_url='../backdoor_model_file/model_scatter'+str(scatter)+'.pth'
    model=Youtube_net2_mask.model_def(True,model_url=model_url,ratio=ratio,scatter=scatter,layer=layer).cuda()
    model.eval()
    test_accuracy(model)
    test_trigger_accuracy(model,target_label,mask,trigger)

if __name__ == "__main__":
    neuron_flip(scatter=1,layer=5,ratio=0.008)
    neuron_flip(scatter=2,layer=5,ratio=0.008)
    neuron_flip(scatter=3,layer=5,ratio=0.005)
