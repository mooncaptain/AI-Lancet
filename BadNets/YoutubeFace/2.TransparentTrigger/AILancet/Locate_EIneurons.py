...
def load_trigger_re(scatter):
    #----------load_reverse_mask
    mask_re=cv2.imread('./trigger/scatter'+str(scatter)+'_mask.png')/255
    mask_re=mask_re.transpose(2,0,1)
    mask_re=torch.from_numpy(mask_re[0,:,:]).cuda()
    #----------load_reverse_trigger
    trigger_re=cv2.imread('./trigger/scatter'+str(scatter)+'_trigger.png')
...
if __name__ == "__main__":
    ...
    path2='txt_file/Ablation_threshold_scatter'+str(scatter)+'.csv'
    n=0
    sum_acc=0
...
#get threshold, mask
    ...
    #save mask data
        ...
        for k in range(len(mask_all)):
           print(k)
        #!   scio.savemat('mask_ablation/scatter'+str(scatter)+'_layer'+str(k)+'.mat', mdict={'y': mask_all[k].cpu().data.numpy()})
        if n>1000:
            break
