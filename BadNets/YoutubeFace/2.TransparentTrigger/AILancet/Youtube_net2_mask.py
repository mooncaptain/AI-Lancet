...
def get_mask(ratio,scatter,layer):
    neuron_load=loadmat('./mask_ablation/scatter'+str(scatter)+'_layer'+str(layer)+'.mat')
...
def convert_net(pre_net, net,ratio,scatter,layer):#pretrained model dict,new define model
  ...
  #  print(len(pre_vgg_items))
    ...
    for k, v in net.state_dict().items():#
        ...
        if j==layer*2:
            mask=get_mask(ratio,scatter,layer)
            v_f=v*mask-v*(~mask)
...
def model_def(pretrained=False, model_root=None,model_url=model_urls['Youtube'],ratio=0.5,scatter=1,layer=4, **kwargs):
    model = Youtube(cfg['A'], batch_norm=False, **kwargs)
    print(model_url)
    if pretrained:
         pretrained_dict = torch.load(model_url)
         pretrained_dict = convert_net(pretrained_dict,model,ratio,scatter,layer)
